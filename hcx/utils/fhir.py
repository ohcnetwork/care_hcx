...
class Fhir:
    ...
    def process_coverage_elibility_check_response(self, response):
        ...
        coverage_eligibility_check_response = (
            coverageeligibilityresponse.CoverageEligibilityResponse(
                **list(
                    filter(
                        lambda entry: isinstance(entry.resource, coverageeligibilityresponse.CoverageEligibilityResponse),
    ...
    def process_claim_response(self, response):
        ...
        claim_response = claimresponse.ClaimResponse(
            **list(
                filter(
                    lambda entry: isinstance(entry.resource, claimresponse.ClaimResponse),
                    claim_bundle.entry,
                )
            )[0].resource.dict()
        )
